...
export const codexAdapter: AgentAdapter<CodexRunState> = {
	...
	handleEvent(event, ctx, state) {
		...
		if (event?.type === "thread.started" && event.thread_id) {
			...
			if (!state.sawAssistantStream) {
				ctx.emitChatEvent({ type: "result", result: event.last_agent_message });
				state.hasFinalAssistantMessage = true;
			}
		} else if (
			eventText &&
	...
	async finalize({ state, exitCode, stderrText, ctx }) {
		...
		if (
			...
			ctx.emitChatEvent({ type: "result", result: assistantText });
