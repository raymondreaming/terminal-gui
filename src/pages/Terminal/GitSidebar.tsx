...
const STATUS_COLORS: Record<string, string> = {
	...
	"?": "text-surgent-text-3 bg-surgent-text/5",
...
function StatusBadge({ status }: { status: string }) {
	const color =
		STATUS_COLORS[status] || "text-surgent-text-3 bg-surgent-text/5";
	return (
		<span
...
function FileRow({
	...
	return (
		...
		>
			...
			<span className="truncate text-[10px] text-surgent-text flex-1">
				{filename}
			</span>
			{dir && (
				<span className="text-[9px] text-surgent-text-3 truncate max-w-[80px]">
					{dir}
				</span>
			)}
		</button>
		...
		<div className="border-b border-surgent-border/50 last:border-b-0">
			...
			>
				<svg
					...
					height="8"
					viewBox="0 0 8 8"
					className={`shrink-0 text-surgent-text-3 transition-transform ${expanded ? "rotate-90" : ""}`}
					fill="currentColor"
				>
					<path d="M2 1l4 3-4 3z" />
				</svg>
				<span className="text-[11px] font-medium text-surgent-text truncate flex-1">
					{project.name}
				</span>
				<span className="text-[9px] font-medium text-surgent-accent bg-surgent-accent/10 px-1.5 py-0.5 rounded">
					{project.branch}
				</span>
				{totalChanges > 0 && (
					<span className="text-[9px] font-semibold text-surgent-text-3 tabular-nums">
						{totalChanges}
					</span>
				)}
			</button>
			...
			{expanded && (
				<div className="pb-1">
					...
					{untrackedFiles.length > 0 && (
						<div className="px-1">
							<div className="px-2 py-0.5 text-[9px] font-semibold text-surgent-text-3/70 uppercase tracking-wider">
					...
					{totalChanges === 0 && (
						<p className="px-3 py-2 text-[10px] text-surgent-text-3">
							Clean working tree
						</p>
					)}
				</div>
...
export const GitSidebar = memo(function GitSidebar({
	...
	return (
		<CollapsibleSidebarSection
			...
			label="Git"
			count={totalChanges}
			countColor={totalChanges > 0 ? "text-yellow-400" : "text-surgent-text-3"}
			expanded={expanded}
			onToggle={onToggle}
